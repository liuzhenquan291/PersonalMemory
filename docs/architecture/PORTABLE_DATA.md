...
M4.7 明确：当前 MVP 不提供可读导入、由可读导出重建 SQLite/FTS/向量索引或从该导出启动新安装的入口。跨安装迁移与灾难恢复必须使用下述完整备份；可读导入和索引重建属于 ADR-0003 的长期方向，完成 schema、冲突规则和迁移 E2E 前不得宣称可用。
导出操作会在文件成功写入后追加一条 `data.exported` 事件，因此本次导出包含此前审计记录，下一次导出才会包含本次导出事件。完整备份保留本地 HMAC 密钥；可读导出故意不携带该密钥，迁移后的脱敏引用不能用来反查原始记忆 ID。
可读导出成功后还会把绝对路径登记为受管副本；若登记失败，刚生成的文件会被删除并让操作失败。用户后续移动、复制或改名会产生产品无法发现的外部副本。
...
完整备份成功切换到目标目录后登记为受管副本；登记失败时删除该新备份。因为备份为明文且含多个事实资产，M3.4 级联删除不会原地改写或伪称密钥销毁，而是在用户确认后校验 manifest 并整件删除登记备份。
...
恢复前目录可能含已删除或已纠正记忆，仍由用户在确认新数据可用后显式处理。产品创建并仍位于登记路径的备份已纳入 M3.4 整件删除；用户移动或复制的备份不在自动发现范围。跨升级回退目录和更长期生命周期编排仍属于 M5.3。

...
M2 个人记忆核心闭环以及 M3.1–M3.3 可信治理已完成：新 L1 记忆默认待审核，只有接受后才进入召回；用户可以设置有效期、标记冲突、人工合并、建立或撤销替代关系，并查看不含正文的本地审计时间线。产品 MVP 尚未完成；M3.4–M5 的可验证级联删除、真实 MCP 客户端接入、一条命令安装、安全升级和发布候选门禁仍待实施。拒绝、替代或受控删除都不等于彻底删除。

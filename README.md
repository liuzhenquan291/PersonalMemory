...
M0–M5 的核心闭环与发布工程基线，以及 M4.5 自动 Agent 生命周期、M4.6 模型/隐私门禁和 M4.7 现有承诺收口均已完成。M5.5 正在用真实源码分发物重新验收：macOS arm64 已通过双客户端自然 Hook、首条自动召回/捕获、升级、默认零外联、模型与 Hook 授权变化、敏感采集、故障/outbox 恢复、重复事件和 50 轮连续运行，并修复了验收发现的干净包启动问题。当前只剩 Linux arm64；本机 Docker Desktop 的 containerd 存储 I/O 故障仍使引擎健康检查超时。项目仍不是发布候选，不移动 `main`、不推送、不对外发布。详见 [MVP 缺口与后续路线](docs/MVP_GAPS_AND_ROADMAP.md)。

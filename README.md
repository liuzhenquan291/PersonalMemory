...
M2 个人记忆核心闭环已通过退出验收，M3.1 记忆审核收件箱也已完成：新 L1 记忆默认待审核，只有接受后才进入召回，并支持修改后接受、带原因拒绝、批量处理和可移植审核状态。产品 MVP 尚未完成；M3.2–M5 的冲突治理、审计、可验证级联删除、真实 MCP 客户端接入、一条命令安装、安全升级和发布候选门禁仍待实施。受控删除或拒绝都不等于彻底删除。

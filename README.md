...
M0–M5 的核心闭环与发布工程基线，以及 M4.5 自动 Agent 生命周期、M4.6 模型/隐私门禁和 M4.7 现有承诺收口均已完成。M5.5 正在用真实源码分发物重新验收：macOS arm64 安装、重复安装、备份恢复、卸载及真实 Codex 五工具会话已通过，并修复了干净包构建顺序、数据目录初始化、Gateway 状态目录和 worker 启动竞态。尚待真实验证 Linux arm64、Claude Code 自然会话、首条自动召回/捕获、升级、默认零外联、模型授权变化、敏感采集策略、故障与 outbox、重复事件和长期运行。项目仍不是发布候选，不移动 `main`、不推送、不对外发布。详见 [MVP 缺口与后续路线](docs/MVP_GAPS_AND_ROADMAP.md)。

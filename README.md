...
M2 个人记忆核心闭环、M3 可信治理、M4 双客户端 MCP 互操作和 M5.1 一条命令源码安装已完成。产品 MVP 尚未完成；安全升级、生命周期命令和发布候选门禁仍待实施。“彻底删除”只承诺可验证的受控范围，不代表全磁盘取证。

## 产品安装

macOS 或 Linux 安装 Node.js 22.19.0 以上版本后，在仓库根目录运行：

```bash
npm run install:product
```

命令会检查环境，缺少依赖时按锁文件获取，随后完成构建、私有数据目录初始化、Gateway/Web 后台启动和双健康检查。模型访问和遥测保持关闭。重复执行只验证已有受管进程和健康状态，不替换记忆数据；失败不会删除既有数据。停止、修复和卸载命令在 M5.3 交付。
